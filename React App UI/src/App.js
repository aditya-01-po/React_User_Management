...
class App extends Component {
    ...
    getUserData () {
        api({
            method: 'get',
            url: '/users/' + encodeURIComponent(this.state.current_user)
        }).then((response) => {
            if (response.data) {
            this.setState({
                user_location: response.data.location,
                user_interests: response.data.interest, // note: API returns "interest" not "interests"
                status_message: 'Data about the logged in user received successfully.'
            });
            }
        })
        .catch((error) => {
   ...
   saveUserData() {
        api({
            method: 'put', // or 'post' if you want to create new
            url: '/users',
            data: {
            email: this.state.current_user,
            interest: this.state.user_interests,   // singular
            ...
            <Container>
                ...
                <Form className="box">
                    ...
                    <Form.Group as={Row}>
                        ...
                        <Col sm="10">
                            <Form.Control placeholder="Enter your location" value={this.state.user_location || ''} onChange={(event) => { this.setState({ user_location: event.target.value,}); }}/> 
                        </Col>
                    </Form.Group>
                    <Form.Group as={Row}>
                        <Form.Label column sm="2">
                        ...
                        <Col sm="10">
                            <Form.Control placeholder="Enter your interests" value={this.state.user_interests || ''} onChange={(event) => { this.setState({ user_interests: event.target.value,}); }}/>
                        </Col>
                    </Form.Group>
